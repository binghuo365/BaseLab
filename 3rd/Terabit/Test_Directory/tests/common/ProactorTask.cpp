...
ProactorTask::run_event_loop(u_int thr_num)
{
  ...
  ACE_Proactor * proactor = get_proactor (thr_num);
...
ProactorTask::create_demultiplexor(void)
{
    ...
    {
        ACE_Proactor * proactor = create_proactor(i);
        if (!proactor)
        {
            return -1;
        }
...
ProactorTask::create_proactor(u_int idx)
{
  ...
  ACE_NEW_RETURN (proactor_impl,
                  ACE_WIN32_Proactor (this->cfg_.threads()),
                  0);
  ...
  switch (ptype)
    ...
    case EPOLL:
      ...
      ACE_NEW_RETURN (proactor_impl,
                      ACE_Event_Poll_Proactor (max_op,
                                          sig_num,
                                          leader_flg),
                      0);
      break;
