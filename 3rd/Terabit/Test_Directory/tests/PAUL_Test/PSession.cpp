...
**
** Contact support@terabit.com.au if any conditions of this licensing
...
PSession::addresses (const ACE_INET_Addr& peer, const ACE_INET_Addr& local)
{
  ...
  if (0 != peer.addr_to_string (str_peer, sizeof (str_peer)/sizeof (ACE_TCHAR)))
    ACE_OS::strcpy (str_peer, ACE_TEXT("(%t) ????"));
...
Acceptor::cancel(void)
{
  ...
  return this->ACE_Asynch_Acceptor<PSession>::cancel ();         
}
...
Connector::cancel(void)
{
  ...
  return this->ACE_Asynch_Connector<PSession>::cancel ();           
}
