...
class Session : public ACE_Service_Handler
{
  friend class Bridge;
...
public:
  ...
  int connect(const ACE_INET_Addr & addr);
...
public:
  ...
  const ACE_INET_Addr & addr_listen (void) const { return this->addr_listen_; }
  const ACE_INET_Addr & addr_connect(void) const { return this->addr_connect_; }
  ...
  int  start(const ACE_INET_Addr & addr_listen,
             const ACE_INET_Addr & addr_connect);
  void stop (void);
...
private:
  ...
  ACE_INET_Addr addr_listen_;
  ACE_INET_Addr addr_connect_;
