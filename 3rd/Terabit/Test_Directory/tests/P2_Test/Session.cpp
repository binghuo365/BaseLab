...
void
Session::addresses (const ACE_INET_Addr& peer, const ACE_INET_Addr& local)
{
...
Session::open_streams (ACE_HANDLE handle)
{
    ...
    ACE_Proactor * proactor = this->bridge ()->acceptor ().proactor();
...
void
Session::handle_read_stream (const ACE_Asynch_Read_Stream::Result &result)
{
  {
    ACE_GUARD (ACE_SYNCH_MUTEX, monitor, this->bridge_->mutex());
...
void
Session::handle_write_stream (const ACE_Asynch_Write_Stream::Result &result)
{
  {
    ACE_GUARD (ACE_SYNCH_MUTEX, monitor, this->bridge_->mutex());
...
int
Sender::connect(const ACE_INET_Addr & addr)
{
  ...
  if (this->asynch_connect_.connect 
                    (ACE_INVALID_HANDLE,
                     addr,                        // remote sap
                     ACE_INET_Addr ((u_short)0),  // local sap
                     1,                           // reuse addr 
                     0) != 0)                     // act
      ACE_ERROR_RETURN 
        ((LM_ERROR,
...
void 
Sender::handle_connect (const ACE_Asynch_Connect::Result &result)
{
  {
    ACE_GUARD (ACE_SYNCH_MUTEX, monitor, this->bridge_->mutex());
...
void
Sender::parse_address (const ACE_Asynch_Connect::Result &result)
{
