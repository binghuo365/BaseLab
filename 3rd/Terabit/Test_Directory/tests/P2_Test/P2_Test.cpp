        ...
        if (this->sender_ != 0)
          {
            ...
            const ACE_INET_Addr & addr = this->acceptor().addr_connect();
...
int
Acceptor::start(const ACE_INET_Addr & addr_listen,
                const ACE_INET_Addr & addr_connect)
{
  ...
  return this->open(this->addr_listen_,
                    ...
                    1,   //int reuse_addr = 1,
                    this->task_.get_proactor(0), // ACE_Proactor *proactor = 0,
  ...
  if (task1.start () == 0)
    {
      ...
      if (acceptor.start (ACE_INET_Addr (cfg.listen_port()),
