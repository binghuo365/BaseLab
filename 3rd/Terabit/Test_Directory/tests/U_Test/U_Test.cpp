...
template class ACE_Asynch_RW<ACE_Asynch_Read_Dgram,ACE_Asynch_Write_Dgram>;
template class ACE_Asynch_RW_Dgram<ACE_Asynch_Read_Dgram,ACE_Asynch_Write_Dgram>;

#elif defined (ACE_HAS_TEMPLATE_INSTANTIATION_PRAGMA)

#pragma instantiate ACE_Asynch_RW<ACE_Asynch_Read_Dgram,ACE_Asynch_Write_Dgram>
#pragma instantiate ACE_Asynch_RW_Dgram<ACE_Asynch_Read_Dgram,ACE_Asynch_Write_Dgram>
...
DSession::print_address (const ACE_Addr& addr)
{
  ...
  const ACE_INET_Addr& in_addr = static_cast<const ACE_INET_Addr&> (addr);

  if (in_addr.addr_to_string (str_addr, 
                              sizeof (str_addr)/sizeof (ACE_TCHAR)) != 0)
    str_addr[0] = 0;
...
DSession::post_message (void)
{
  ...
  ACE_Asynch_User_Result result (this, 
...
void
DSession::handle_read_dgram (const ACE_Asynch_Read_Dgram::Result &result)
{
  {
    ACE_GUARD (ACE_SYNCH_MUTEX, monitor, this->lock_ );
...
void
DSession::handle_write_dgram(const ACE_Asynch_Write_Dgram::Result &result)
{
  {
    ACE_GUARD (ACE_SYNCH_MUTEX, monitor, this->lock_);
...
void
DSession::handle_user_operation(const ACE_Asynch_User_Result& result)
{
...
int
Receiver::on_open (const ACE_INET_Addr & local,
                   const ACE_INET_Addr & remote)
{
   ACE_UNUSED_ARG(local);
   ACE_UNUSED_ARG(remote);

   return this->initiate_read();
}
...
Sender::Sender (DOwner & owner, int index)
  : DSession (owner, index, ACE_TEXT("SNDR"))
{
}
