...
{ 
  ...
  do
    {
      new_handle = ACE_OS::accept (listen_handle, 0, 0);
    ...
    case 0:
      {
        ...
        ACE_Addr & local_address  = original_.local_address();
        ACE_Addr & remote_address = original_.remote_address();
        ACE_SOCK_Stream stream (new_handle);
        stream.get_local_addr (local_address);
        stream.get_remote_addr (remote_address);
      }
      break;
    default:
      break;
...
{ 
  ...
  for (;;)
  {
      int rc = ACE_OS::connect 
                (this->original_.handle (),
                ACE_reinterpret_cast (sockaddr *, addr.get_addr ()),
                addr.get_size ());
