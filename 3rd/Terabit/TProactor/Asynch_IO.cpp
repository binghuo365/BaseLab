...
// ***********************************************************************
static int setAddress(ACE_Addr & dst, const ACE_Addr& src)
{
  ...
  if (dst_type == src_type)
    {
      dst.set_addr(src.get_addr (), src.get_size ());
      return 0;
    }

#if defined (ACE_HAS_IPV6)
  if ((src_type == AF_INET || src_type == AF_INET6) &&
      (dst_type == AF_INET || dst_type == AF_INET6))
#endif
    {
      (ACE_INET_Addr&) dst = (const ACE_INET_Addr&) src;
      return 0;
    }
  return -1;
}

static int setAddress(ACE_Addr & dst, const sockaddr *src, int size)
{
...
TRB_Asynch_Accept_Result::update_addresses (sockaddr   *local_addr,
                                            ...
                                            int         remote_size)
{
  setAddress (this->local_address_, local_addr, local_size);
  setAddress (this->remote_address_, remote_addr, remote_size);
}
  ...
  , local_address_    ()
{
  ...
  ACE_ASSERT (remote_sap.get_type() == PF_INET); 

  setAddress (this->local_address_, local_sap);
  setAddress (this->remote_address_, remote_sap);
}
  ...
  if (this->success ()) 
    {
       ...
       stream.get_local_addr (this->local_address_);
    }
  else
    {
      // failure
...
int 
TRB_Asynch_Read_Dgram_Result::remote_address (ACE_Addr& addr) const
{
  return setAddress (addr, this->remote_address_);
}
...
int 
TRB_Asynch_Write_Dgram_Result::remote_address (ACE_Addr& addr) const
{
  return setAddress (addr, this->remote_address_);
}
...
int
TRB_Asynch_Connect::prepare_connect (ACE_HANDLE &     connect_handle,
                               const ACE_Addr & remote_sap,
                               const ACE_Addr & local_sap,
                               int  reuse_addr)
{
  ...
  if (local_sap != ACE_Addr::sap_any)
    ...
    {
       if (ACE::bind_port (connect_handle) == -1)
        {
           ACE_ERROR_RETURN ((LM_ERROR,
             ACE_LIB_TEXT("%N:%l:TRB_Asynch_Connect::prepare_connect:")
...
{
  // only supporting INET addresses
  ACE_ASSERT (protocol_family == AF_INET);
...
TRB_Service_Handler::TRB_Service_Handler (void)
{
}
...
void
TRB_Service_Handler::addresses (const ACE_INET_Addr & /* remote_address */,
                                const ACE_INET_Addr & /* local_address */ )
{
}

void
TRB_Service_Handler::act (const void *)
{
}

void
TRB_Service_Handler::open (ACE_HANDLE,
                           ACE_Message_Block &)
{
}
