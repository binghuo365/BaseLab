...
//AL: commented 
//AL: see TRB_Asynch_Acceptor<HANDLER>::cancel (void)
...
//AL: see TRB_Asynch_Acceptor<HANDLER>::cancel (void)
    ...
    TRB_Asynch_Acceptor<Test_Service_Handler>::cancel();
...
//AL: I remommend replace the logic
    ...
    //    reissue_accept(0);
    //return TRB_Asynch_Acceptor<Test_Service_Handler>::accept(bytes_to_read, act);

    if  (cancel_flag_!= 0)  // AL: no need in atomic value())
      return -1;

    accept_cnt_++;
    int rc = TRB_Asynch_Acceptor<Test_Service_Handler>::accept(bytes_to_read, act);
    if (rc != 0)
      accept_cnt_--;
...
//    by TRB_Asynch_Acceptor<Test_Service_Handler>::cancel();
//
//    if (cancel_flag_.value())
//        reissue_accept(0);

    TRB_Asynch_Acceptor<Test_Service_Handler>::handle_accept(result);
