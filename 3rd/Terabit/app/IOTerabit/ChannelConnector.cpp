...
int
ChannelConnector::start (const ACE_INET_Addr& addr, const void *act)
{
...
ChannelConnector::on_connect_completed (const TRB_Asynch_Connect::Result&  result)
{
    ...
    if (this->get_log_level () < 2)
    {
        ...
        remote_addr.addr_to_string (str_peer, sizeof (str_peer)/sizeof (ACE_TCHAR));
        local_addr.addr_to_string (str_local, sizeof (str_local)/sizeof (ACE_TCHAR));
